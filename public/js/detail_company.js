$(function () {
    ...
    // 添加滚动条
    var height = parseInt($(".scroll").parent().css("height"));
    $(".scroll").css("height",height - 40 + "px");
    $(".scroll").mCustomScrollbar();

    $(".selfCheck td:first-child").each(function (i,v) {
        var html = v.innerHTML;
        $(".selfCheck td:first-child")[i].innerHTML = html.length > 11 ? (html.slice(0,10) + '...') : html;
        $($(".selfCheck td:first-child")[i]).attr('title',html);
    });
    $(".otherCheck td:first-child").each(function (i,v) {
        var html = v.innerHTML;
        $(".otherCheck td:first-child")[i].innerHTML = html.length > 11 ? (html.slice(0,10) + '...') : html;
        $($(".otherCheck td:first-child")[i]).attr('title',html);
    });
    /*************变量定义*************/
    var mapBox = $('#map');//地图容器
    /***************************地图定义区begin**********************/
    ...
    var companyIcon= L.divIcon({//房屋图标
    ...
    if ($('#lat').html()) {
        L.marker([$('#lat').html(),$('#lng').html()],{icon:companyIcon}).addTo(map);
