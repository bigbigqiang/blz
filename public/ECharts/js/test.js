...
function renderBrushed(params) {
    ...
    this.setOption({
		...
		{
            id: 'statistic',
            text: count ? '最大污染指数 : ' + (sum / count).toFixed(4)+'   \t 最小污染指数:'+(sum / count/5).toFixed(4)+'   \t 平均空气质量:'+(sum / count/3.5).toFixed(4): ''
        }],
        series: [
		{
            id: 'bar',
        ...
        },
		{
            ...
            data: [
                {
                    value: [Math.round(Math.random()*100), Math.round(Math.random()*100), Math.round(Math.random()*100), Math.round(Math.random()*100), Math.round(Math.random()*100), Math.round(Math.random()*100)],
                    name: '地区天气'
                }
            ]
        },]
...
var clouds=['少云','局部多云','多云','低空少云','低空多云','全境无云','高空少云'];

function cityWeather(params){
		if(params.seriesType=="effectScatter"||params.seriesType=="scatter"){
			$('#details').show();
			$(".city").text(params.name);
			$(".channel").text(params.name+"卫视");
			var i=Math.floor(Math.random()*7);
			var temperature=((Math.random()+1.5)*10).toFixed(1);
			var humidity=((Math.random()*3+6)*10).toFixed(0);
			var pressure=(Math.random()*30+1000).toFixed(1);
			var seaPressure=((Math.random()*30+1000)*1.013).toFixed(1);
			var orientation=['东风','西风','南风','北风','东南风','东北风','西南风','西北风',];
			var wind=(Math.random()*36).toFixed(2);
			var j=Math.floor(Math.random()*8);
			var visibility=['小于 3 千米','3~5 千米','5~10 千米','10~20 千米','20~30 千米','30~50 千米','大于 50 千米'];
			var k=Math.floor(Math.random()*7);
			var cloudHeight=((Math.random()*20+28)*100).toFixed(1);
			$(".temperature").text(temperature+"°C");
			$(".humidity").text(humidity+"%");
			$(".pressure").text(pressure+"百帕 （海洋大气压: "+seaPressure+"百帕）");
			$(".winds").text(orientation[j]+" "+wind+"千米/小时");
			$(".visibility").text(visibility[k]);
			$(".cloudHeight").text("大于 "+cloudHeight+" 米");
			$(".clouds").text(clouds[i]);
			var linedata = [Math.random() * 300];
			var temp;
			for (var i = 1; i < 100; i++) {
				temp=Math.round((Math.random() - 0.5) * 30 + linedata[i - 1])
				var nextCount;
				if(temp>=300){
					nextCount=Math.abs((temp-30)%300);
					
				}else{
					nextCount=Math.abs(temp%300);
					
				}
				linedata.push(nextCount);
			}	
			this.setOption({
			 series:
			{
...
myChart.on('mouseover', cityWeather);
